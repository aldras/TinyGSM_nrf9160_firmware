...
MEMORY
    {
    FLASH (rx) : ORIGIN = 0x18200, LENGTH = 0x5ce00
    SRAM (wx) : ORIGIN = 0x20020000, LENGTH = 0x20000
    IDT_LIST (wx) : ORIGIN = (0x20020000 + 0x20000), LENGTH = 2K
    }
ENTRY("__start")
SECTIONS
    {
 ...
 *(.ARM.exidx* gnu.linkonce.armexidx.*)
 ...
 net_socket_register_area : SUBALIGN(4) { _net_socket_register_list_start = .; KEEP(*(SORT(._net_socket_register.static.*))); _net_socket_register_list_end = .; } > FLASH
 ...
 *(".kernel.*")
 ...
 devices : ALIGN_WITH_INPUT
 {
  ...
  __device_end = .;
  FILL(0x00); __device_init_status_start = .; . = . + (((((__device_end - __device_start) / 0x18) + 31) / 32) * 4); __device_init_status_end = .;
  FILL(0x00); __device_busy_start = .; . = . + (((((__device_end - __device_start) / 0x18) + 31) / 32) * 4); __device_busy_end = .;
 } > SRAM AT> FLASH
 ...
 net_if_area : ALIGN_WITH_INPUT SUBALIGN(4) { _net_if_list_start = .; KEEP(*(SORT(._net_if.static.*))); _net_if_list_end = .; } > SRAM AT> FLASH
 net_if_dev_area : ALIGN_WITH_INPUT SUBALIGN(4) { _net_if_dev_list_start = .; KEEP(*(SORT(._net_if_dev.static.*))); _net_if_dev_list_end = .; } > SRAM AT> FLASH
 net_l2_area : ALIGN_WITH_INPUT SUBALIGN(4) { _net_l2_list_start = .; KEEP(*(SORT(._net_l2.static.*))); _net_l2_list_end = .; } > SRAM AT> FLASH
    __data_ram_end = .;
 ...
 *(".kernel_noinit.*")
    ...
    _image_ram_end = .;
    _end = .;
    __kernel_ram_end = 0x20020000 + 0x20000;
